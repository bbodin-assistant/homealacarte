...
test_port="${HOMEALACARTE_SPENDING_TEST_PORT:-18082}"
chrome_port="${HOMEALACARTE_SPENDING_CHROME_PORT:-9224}"
origin="http://127.0.0.1:${test_port}"
browser_profile="/tmp/homealacarte-spending-browser-profile-$$"
python_executable="${PYTHON_EXECUTABLE:-python3}"

"${python_executable}" -m http.server "${test_port}" --bind 127.0.0.1 --directory dist \
  >/tmp/homealacarte-spending-server.log 2>&1 &
test_server_pid=$!
google-chrome --headless --no-sandbox --disable-gpu \
  --remote-debugging-port="${chrome_port}" --remote-allow-origins=* \
  --user-data-dir="${browser_profile}" about:blank \
  >/tmp/homealacarte-spending-chrome.log 2>&1 &
test_chrome_pid=$!
...
node tests/grocery_spending_browser.mjs "${origin}" "${chrome_port}"
