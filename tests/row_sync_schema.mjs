...
assert.match(
  schema,
  /household_changes_record_lookup_idx[\s\S]*?\(user_id, entity_type, entity_id, change_id desc\)/,
);
...
console.log("Supabase schema defines versioned rows, indexed change lookup, a durable cursor log, idempotency, and RLS.");
