...
Locale keys use language tags such as `en`, `fr`, `es`, `zh-CN`, `en-GB`, or `fr-FR`. Localized data is not restricted to the languages currently translated by the application UI. Resolution first tries the requested locale, then its base language, then English as a neutral fallback, then the first available value. A normal JSON string remains language-neutral and is fully backward compatible.
...
When the language changes, source-backed localized text is switched while runtime changes such as edited quantities, added or deleted records, and user-edited text are preserved. For structural day and meal labels, French is used for `fr` locales and English is currently the neutral fallback for other locales until additional structural labels are modeled.
