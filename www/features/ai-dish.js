...
const STRINGS = {
  en: {
    ...
    intro: "Paste a recipe, ingredient list, or cooking notes. A local Ollama model will structure one dish and add it to your database.",
    recipe: "Recipe text",
    recipePlaceholder: "Paste the recipe here…",
    server: "Ollama server",
    model: "Model",
    refresh: "Refresh models",
    privacyLocal: "The recipe is sent directly from this browser to your local Ollama server.",
    privacyRemote: "This server is not local. The pasted recipe text will be sent to that server.",
    corsHint: "If this site cannot reach local Ollama, allow this page origin with OLLAMA_ORIGINS and restart Ollama.",
    cancel: "Cancel",
    stop: "Stop",
    submit: "Generate and add dish",
    loadingModels: "Looking for installed Ollama models…",
    generating: "Generating structured recipe… {seconds}s",
    noModels: "No Ollama models were found on this server.",
    stopped: "Generation stopped.",
    invalidUrl: "Check the Ollama server address.",
    network: "Cannot reach Ollama. Check that it is running and that this site is allowed by OLLAMA_ORIGINS.",
    timeout: "Ollama took too long to respond. Try a faster/smaller model or a shorter recipe.",
    ...
    modelRequired: "Select an Ollama model.",
  },
  fr: {
    add: "Ajouter avec l’IA",
    title: "Ajouter un plat avec l’IA",
    intro: "Collez une recette, une liste d’ingrédients ou des notes. Un modèle Ollama local structurera un plat et l’ajoutera à votre base.",
    recipe: "Texte de la recette",
    recipePlaceholder: "Collez la recette ici…",
    server: "Serveur Ollama",
    model: "Modèle",
    refresh: "Actualiser les modèles",
    privacyLocal: "La recette est envoyée directement par ce navigateur à votre serveur Ollama local.",
    privacyRemote: "Ce serveur n’est pas local. Le texte collé sera envoyé à ce serveur.",
    corsHint: "Si ce site ne peut pas joindre Ollama en local, autorisez l’origine de cette page avec OLLAMA_ORIGINS puis redémarrez Ollama.",
    cancel: "Annuler",
    stop: "Arrêter",
    submit: "Générer et ajouter le plat",
    loadingModels: "Recherche des modèles Ollama installés…",
    generating: "Génération de la recette structurée… {seconds}s",
    noModels: "Aucun modèle Ollama n’a été trouvé sur ce serveur.",
    stopped: "Génération arrêtée.",
    invalidUrl: "Vérifiez l’adresse du serveur Ollama.",
    network: "Impossible de joindre Ollama. Vérifiez qu’il fonctionne et que ce site est autorisé par OLLAMA_ORIGINS.",
    timeout: "Ollama a mis trop de temps à répondre. Essayez un modèle plus rapide/petit ou une recette plus courte.",
    ...
    modelRequired: "Sélectionnez un modèle Ollama.",
  },
};
...
export function buildDishSavePayload(recipe, snapshot) {
  ...
  return {
    dish: {
      ...
      source: text(recipe.source, 240) || "Ollama AI recipe import",
      nutri_score: "",
    ...
    documentRef.body.insertAdjacentHTML("beforeend", `
      <dialog id="ai-dish-dialog" class="menu-item-dialog ai-dish-dialog" aria-labelledby="ai-dish-title">
        <form id="ai-dish-form" class="ai-dish-form">
          <div class="menu-dialog-heading"><div><p class="eyebrow">AI · Ollama</p><h2 id="ai-dish-title"></h2><p id="ai-dish-intro"></p></div>
