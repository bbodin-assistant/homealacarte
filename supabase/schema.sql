...
-- Remove database objects from the retired per-entity synchronization design.
-- Their contents are deliberately not converted: household_state is the only
-- supported online data format.
drop function if exists public.get_household_sync_snapshot();
drop function if exists public.validate_household_sync_state(uuid);
drop function if exists public.apply_household_sync_operations(jsonb);
drop function if exists public.household_document_from_rows(uuid);
drop function if exists public.log_household_record_change();
drop table if exists public.household_sync_operations cascade;
drop table if exists public.household_changes cascade;
drop table if exists public.household_records cascade;
...
-- Existing users are deliberately treated as active when the lifecycle table
-- is first introduced. Consent fields stay empty until actually recorded.
...
-- Delete the private household document, privacy requests, lifecycle data, and
-- the Auth account in one transaction. Immediate deletion avoids depending on
-- a Cron job that cannot run while a Supabase Free project is paused.
